...
router.post('/upload-files', upload.fields([
    ...
    const insertSql = `
        ...
        (national_id_card_or_passport, selfie, proof_of_address)
        VALUES (?, ?, ?)
    `;

    db.query(insertSql, [filePaths.national_id_card_or_passport, filePaths.selfie, filePaths.proof_of_address], function (err, result) {
        if (err) {
